  ...
  changes: Array<{
    value: {
      ...
      statuses?: Array<{
         ...
         * explanation. Persisted on messages.error_* by handleStatusUpdate
         * (migration 028 + 060). See #535.
         */
...
async function handleStatusUpdate(status: {
  ...
  //    assume a single row.
  //
  //    On status='failed', persist Meta's failure code + title + details
  //    (migration 028 + 060, see #535) so the inbox can render the actual
  //    reason instead of a generic red X. `failure` is already computed
  //    above from status.errors[0].
  const messageUpdate: Record<string, unknown> = { status: status.status }
  if (failure) {
    // Stringify the code — migration 028 declared error_code TEXT
    // and migration 060's ADD COLUMN IF NOT EXISTS is a no-op on
    // that pre-existing column, so the type never migrated to INTEGER.
    messageUpdate.error_code = String(failure.code)
