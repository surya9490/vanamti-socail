...
describe('status webhook: failed statuses keep Meta\'s reason (#535)', () => {
  ...
  it('persists code, title and details on the messages row in the same update as status', async () => {
    ...
    expect(h.state.messageUpdates[0]).toEqual({
      status: 'failed',
      // Stringified: our error_code column is TEXT (migration 028 predates
      // upstream 060 which reserved INTEGER — since ADD COLUMN IF NOT EXISTS
      // is a no-op on the existing TEXT column, we keep the wire format
      // consistent with the DB shape by stringifying here.
      error_code: '131049',
