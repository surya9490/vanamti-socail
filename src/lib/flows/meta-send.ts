...
export interface SendCarouselTemplateEngineArgs {
  ...
  bodyParams?: string[]
  cards: CarouselCard[]
  /** Human-readable summary for the inbox conversation-list preview. */
  summaryText: string
}
...
/**
 * Send a WhatsApp Carousel Template Message. Same account-scoped
 * lookup + retry pattern as engineSendProductList.
 *
 * Template must already be approved by Meta. Cost: MARKETING
 * template, billed per send (~₹0.85 in India). Prefer
 * engineSendProductList when the customer is in their 24hr
 * session (free).
 */
export async function engineSendCarouselTemplate(
  args: SendCarouselTemplateEngineArgs,
): Promise<{ whatsapp_message_id: string }> {
  const db = supabaseAdmin()
  ...
  const attempt = async (phone: string): Promise<string> => {
    const r = await sendCarouselTemplateMessage({
      phoneNumberId: config.phone_number_id,
      accessToken,
      to: phone,
  ...
  await db
    .from('conversations')
    .update({
      last_message_text: args.summaryText,
      last_message_at: new Date().toISOString(),
    })
    .eq('id', args.conversationId)
