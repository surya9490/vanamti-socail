...
export function entryTriggerTexts(message: ParsedInbound): string[] {
  ...
  // The union recently gained an `image` variant (upstream migration
  // 039 inbound_media_mirror). Only interactive_reply carries the
  // reply title/id used for keyword matching; media messages have no
  // matchable text on this codepath, so return empty. Narrows the
  // switch instead of relying on an implicit else.
  ...
  if (action.type === "reprompt") {
    ...
    try {
      if (currentNode.node_type === "send_buttons") {
        ...
        // Customer sent text/tapped instead of an image — re-ask for
        // the picture (e.g. the payment screenshot).
        const cfg = currentNode.config as unknown as AwaitImageNodeConfig;
        await engineSendText({
          accountId: run.account_id,
          userId: run.user_id,
