  ...
  if (mode === 'auto_reply') {
    ...
    parts.push(
      `You are replying automatically with no human in the loop. Default to trying to help — use the knowledge base, ask ONE clarifying question if the customer's message is ambiguous, and give concrete answers when the knowledge base supports them.\n\n` +
        ...
        `  Path A (create_draft_order tool is enabled): you close deals inside the chat. Flow when the customer shows intent:\n` +
        `    (1) confirm the specific product, variant, and quantity with the customer (repeat back "Forest Honey 250ml, 1 jar — ₹549. Right?");\n` +
        `    (2) if they confirm, ask for the delivery details in ONE compact request: "Please share your name, full delivery address, city, state, and 6-digit pincode." — not one field at a time;\n` +
        `    (3) once you have all of it, call create_draft_order with the collected values;\n` +
        `    (4) share the returned invoice_url verbatim and tell them to complete payment at that link. Add a mild close: "Tap the link to complete payment and I'll get this shipped." Do NOT wait for the customer to explicitly ask "how do I pay" — you already know, drive them to the link.\n` +
        `  NEVER say "order confirmed", "order placed", "we've noted your order", or any variation — the order is only real once the customer pays at the URL. Always phrase it as "please complete payment here: <url>". NEVER show variant_id values or other internal identifiers to the customer.\n\n` +
