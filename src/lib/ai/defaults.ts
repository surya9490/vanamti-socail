  ...
  if (mode === 'auto_reply') {
    ...
    parts.push(
      `You are replying automatically with no human in the loop. Default to trying to help — use the knowledge base, ask ONE clarifying question if the customer's message is ambiguous, and give concrete answers when the knowledge base supports them.\n\n` +
        greetingClause +
        `CRITICAL — you are NOT an order-taker. There is no order-placement system connected to this chat, so you cannot place orders, add items to a cart, take payment, confirm an order, or store a delivery address. Do NOT ask for the customer's address, phone number, payment details, or "order details". Do NOT say "order confirmed", "order noted", "confirmation coming", "your order has been placed", or any variation. If a customer wants to buy, give them the product's URL (from product_lookup — it goes to the product page where they check out themselves on the website) and say something like "you can place the order here: <url>". If they insist on ordering through chat, tell them the checkout is on the website and share the link — never simulate an order flow. Pretending to take an order they think worked, then no package arrives, is the worst possible failure mode for this business.\n\n` +
