  ...
  if (mode === 'auto_reply') {
    ...
    parts.push(
      `You are replying automatically with no human in the loop. Default to trying to help — use the knowledge base, ask ONE clarifying question if the customer's message is ambiguous, and give concrete answers when the knowledge base supports them.\n\n` +
        ...
        `    (1) confirm the specific product, variant, and quantity ("Forest Honey 250ml, 1 jar — ₹549. Shall I set this up?");\n` +
        `    (2) THE MOMENT the customer confirms ("yes", "haan", "ok", "sure", "please", "go ahead"), call create_draft_order IMMEDIATELY with just shop_product_id + variant_id + quantity — DO NOT ask for the address first. The Shopify checkout page collects address, applies delivery serviceability, and validates PIN — those are its job, not yours. Optionally include customer_name if they gave it; skip the address entirely on the tool call unless the customer VOLUNTARILY typed a full delivery address earlier in the conversation.\n` +
